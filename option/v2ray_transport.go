...
func checkV2RayXHTTPBaseOptions(mode string, options *V2RayXHTTPBaseOptions) error {
	...
	case "":
		options.XPaddingPlacement = PlacementQueryInHeader
	...
	case "":
		options.UplinkDataPlacement = PlacementBody
	case PlacementBody:
	...
	case "":
		options.SessionPlacement = PlacementPath
	...
	case PlacementCookie, PlacementHeader, PlacementQuery:
		if options.SessionPlacement == PlacementPath {
	...
	if options.SessionPlacement != PlacementPath && options.SessionKey == "" {
		switch options.SessionPlacement {
	...
	if options.SeqPlacement != PlacementPath && options.SeqKey == "" {
		switch options.SeqPlacement {
		case PlacementCookie, PlacementQuery:
